...
function getSessionPatientProfile() {
  try {
    ...
    if (u.role === "patient") {
      return {
        name: (u.name && String(u.name).trim()) || "Patient",
        patientId: u.patientId || getStoredPatientId(),
        fromSession: true,
      };
    }
  } catch (_) {}
  return { name: "Patient", patientId: getStoredPatientId(), fromSession: false };
}
...
function generateToken() {
  return `MCT-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
}

function Avatar({ name, size = 48 }) {
  const safeName = name || "Unknown";
  const initials = safeName.split(" ").filter(w => w !== "Dr.").map(w => w[0]).join("").substring(0, 2);
  const colors = [
    ["#0e3a5c", "#00d4ff"], ["#2d1a5c", "#7c3aed"], ["#0a3d2e", "#10b981"],
    ["#3d2200", "#f59e0b"], ["#3d0a1a", "#ef4444"],
  ];
  const pick = colors[(safeName.charCodeAt(4) || 0) % colors.length];
  return (
    <div style={{
        ...
        {tags.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
              {tags.map(tag => <span key={tag} style={{ background: `${COLORS.accent2}20`, color: "#a78bfa", border: `1px solid ${COLORS.accent2}30`, padding: "4px 12px", borderRadius: 20, fontSize: 12, fontWeight: 600 }}>{tag}</span>)}
            </div>
          </div>
        )}
        {languages.length > 0 && (
          <div style={{ marginBottom: 20 }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {languages.map(lang => <span key={lang} style={{ background: `${COLORS.accent}10`, color: COLORS.accent, border: `1px solid ${COLORS.accent}25`, padding: "4px 12px", borderRadius: 20, fontSize: 12 }}>🗣️ {lang}</span>)}
            </div>
          </div>
        )}
        {reviews.length > 0 && (
...
function PaymentModal({ doctor, time, date, isEmergency, onClose, onSuccess }) {
  ...
  const handlePay = async () => {
    ...
    try {
      ...
      const newToken = data.blockchain || data.blockchainTokenId
        ? `APT-${String(data.blockchainTokenId || "").padStart(4, "0")}`
        : generateToken();
      setToken(newToken);
    } catch {
      ...
      <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center", padding: 24 }}>
        <div onClick={e => e.stopPropagation()} style={{ background: COLORS.card, border: `1px solid ${COLORS.green}40`, borderRadius: 20, padding: 32, maxWidth: 440, width: "100%", textAlign: "center" }}>
          ...
          <div style={{ background: COLORS.bg, border: `1px solid ${COLORS.accent}40`, borderRadius: 14, padding: 20, marginBottom: 24 }}>
            ...
            <div style={{ fontFamily: "monospace", fontSize: 28, fontWeight: 800, color: COLORS.accent, letterSpacing: 4, marginBottom: 16, background: `${COLORS.accent}10`, padding: "10px 20px", borderRadius: 10, border: `1px solid ${COLORS.accent}30` }}>{token}</div>
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10, textAlign: "left" }}>
              {[
                { label: "Doctor", value: doctor.name },
                { label: "Specialty", value: doctor.specialty },
                { label: "Time", value: time },
                { label: "Date", value: date || new Date().toLocaleDateString("en-IN") },
    ...
    return (
      <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.8)", zIndex: 300, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ textAlign: "center", color: COLORS.text }}>
          ...
          <p style={{ color: COLORS.muted, fontSize: 13 }}>Booking appointment on blockchain</p>
        </div>
      </div>
    );
  }
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: COLORS.card, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 20, padding: 28, maxWidth: 420, width: "100%" }}>
        ...
        {method === "card" && (
          <div style={{ display: "flex", flexDirection: "column", gap: 10, marginBottom: 20 }}>
            <input defaultValue="•••• •••• •••• 4242" style={{ background: COLORS.bg, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 10, padding: "10px 14px", color: COLORS.text, fontSize: 13, outline: "none", width: "100%" }} />
            <input defaultValue="John Doe" style={{ background: COLORS.bg, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 10, padding: "10px 14px", color: COLORS.text, fontSize: 13, outline: "none", width: "100%" }} />
        ...
        {method === "upi" && <input defaultValue="john@upi" style={{ width: "100%", background: COLORS.bg, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 10, padding: "10px 14px", color: COLORS.text, fontSize: 13, outline: "none", marginBottom: 20 }} />}
        {method === "netbanking" && (
          <select style={{ width: "100%", background: COLORS.bg, border: `1px solid ${COLORS.cardBorder}`, borderRadius: 10, padding: "10px 14px", color: COLORS.text, fontSize: 13, outline: "none", marginBottom: 20 }}>
...
function AISummaryCard({ summary }) {
  ...
  const recommendedSteps = Array.isArray(summary.recommendedSteps) ? summary.recommendedSteps : [];
  return (
...
export function PatientDashboard() {
  ...
  useEffect(() => {
    const syncSession = () => setSessionPatient(getSessionPatientProfile());
    syncSession();
  ...
  useEffect(() => {
    ...
    const load = () => {
      ...
      Promise.all([
        ...
        fetch(`${API}/records/${encodeURIComponent(pid)}`, { headers }).then(r => r.json()).catch(() => []),
        fetch(`${API}/appointments?patientId=${encodeURIComponent(pid)}`, { headers }).then(r => r.json()).catch(() => []),
      ]).then(([docs, recs, appts]) => {
        if (cancelled) return;
        setDoctors(Array.isArray(docs) && docs.length ? docs : mockDoctors);
        const safeRecs = Array.isArray(recs) ? recs : mockReports;
        setReports(safeRecs);
  ...
  const handleVerifyReport = async (report, idx) => {
    ...
    try {
      const ok = await verifyRecord(getChainPatientId(), report);
  ...
  const handlePaymentSuccess = (token) => {
    const entry = { token, doctor: selectedDoctor, time: selectedTime, date: selectedDate, isEmergency, createdAt: new Date().toLocaleDateString("en-IN") };
  ...
  const specialties = ["All", ...new Set(displayDoctors.map(d => d.specialty).filter(Boolean))];
  ...
  return (
    <div style={{ minHeight: "100vh", background: COLORS.bg, fontFamily: "'Segoe UI', sans-serif" }}>
      ...
      {showPayment && selectedDoctor && selectedTime && (
        <PaymentModal doctor={selectedDoctor} time={selectedTime} date={selectedDate} isEmergency={isEmergency} onClose={() => setShowPayment(false)} onSuccess={handlePaymentSuccess} />
      )}
      ...
      <div style={{ maxWidth: 1280, margin: "0 auto", padding: "32px 24px" }}>
          ...
          <p style={{ color: COLORS.muted }}>
            ...
            {sessionPatient.patientId && (
              <span style={{ display: "block", fontSize: 12, marginTop: 6 }}>
                Patient ID: <span style={{ fontFamily: "monospace", color: COLORS.accent }}>{sessionPatient.patientId}</span>
                {" · "}On-chain ID: <span style={{ fontFamily: "monospace" }}>{getChainPatientId()}</span>
              </span>
            )}
          </p>
        </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 20, marginBottom: 28 }}>
          ...
          <div style={cardStyle}>
            ...
            <p style={{ color: COLORS.muted, fontSize: 13, marginBottom: 12 }}>Browse doctors and book your slot</p>
            ...
            <div style={{ display: "grid", gap: 12, marginBottom: 20 }}>
              {filteredDoctors.map(doc => {
                ...
                const docAvailability = Array.isArray(doc.availability) ? doc.availability : [];
                ...
                return (
                  <div key={docId} style={{ borderRadius: 14, border: `1px solid ${isSelected ? COLORS.accent : COLORS.cardBorder}`, background: isSelected ? `${COLORS.accent}06` : COLORS.bg, overflow: "hidden" }}>
                    <div onClick={() => { setSelectedDoctor({ ...doc, id: docId }); setSelectedTime(""); }} style={{ padding: 14, cursor: "pointer", display: "flex", gap: 14, alignItems: "flex-start" }}>
                      <Avatar name={doc.name || "Unknown"} size={52} />
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                          <div>
                            ...
                            <p style={{ color: COLORS.muted, fontSize: 12 }}>{doc.experience} yrs · {doc.education}</p>
                          </div>
                        ...
                        <div style={{ display: "flex", gap: 5, flexWrap: "wrap", marginTop: 8 }}>
                          {docTags.slice(0, 3).map(tag => <span key={tag} style={{ background: `${COLORS.accent2}15`, color: "#a78bfa", border: `1px solid ${COLORS.accent2}25`, padding: "2px 8px", borderRadius: 20, fontSize: 10, fontWeight: 600 }}>{tag}</span>)}
                          {docLanguages.length > 0 && <span style={{ color: COLORS.muted, fontSize: 10, alignSelf: "center" }}>🗣️ {docLanguages.join(", ")}</span>}
                        </div>
                      </div>
                    ...
                    <div style={{ padding: "0 14px 12px", display: "flex", gap: 8 }}>
                      <button onClick={e => { e.stopPropagation(); setModalDoctor({ ...doc, id: docId }); }} style={{ padding: "5px 12px", borderRadius: 8, fontSize: 12, cursor: "pointer", border: `1px solid ${COLORS.cardBorder}`, background: "transparent", color: COLORS.muted }}>👤 View Full Profile</button>
                      <span style={{ padding: "5px 10px", borderRadius: 8, fontSize: 11, background: `${COLORS.green}15`, color: COLORS.green }}>🟢 {docAvailability.length} slots today</span>
                    </div>
                    ...
                    {isSelected && (
                      <div style={{ borderTop: `1px solid ${COLORS.cardBorder}`, padding: 14, background: `${COLORS.accent}04` }}>
                        ...
                        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, padding: "10px 14px", background: COLORS.bg, borderRadius: 10, border: `1px solid ${COLORS.cardBorder}` }}>
                          ...
                          <div onClick={() => setIsEmergency(!isEmergency)} style={{ width: 44, height: 24, borderRadius: 12, position: "relative", cursor: "pointer", background: isEmergency ? COLORS.red : COLORS.cardBorder, transition: "background 0.2s" }}>
                        ...
                        {isEmergency && <div style={{ padding: 10, background: `${COLORS.red}11`, border: `1px solid ${COLORS.red}33`, borderRadius: 10, color: COLORS.red, fontSize: 12, marginBottom: 12 }}>🚨 Emergency adds 50% surcharge (₹{doc.fee} → ₹{Math.round(doc.fee * 1.5)}). You'll be placed #1 in queue.</div>}
                        {selectedTime ? (
                          <button onClick={() => setShowPayment(true)} style={{ width: "100%", padding: "12px", background: `linear-gradient(135deg, ${COLORS.accent}, ${COLORS.accent2})`, border: "none", color: "#fff", fontSize: 15, fontWeight: 700, borderRadius: 10, cursor: "pointer" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {specialties.filter(s => s !== "All").map(spec => {
                  ...
                  return (
                    <div key={spec} onClick={() => setFilterSpec(spec)} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 12px", borderRadius: 10, cursor: "pointer", background: filterSpec === spec ? `${COLORS.accent}10` : COLORS.bg, border: `1px solid ${filterSpec === spec ? COLORS.accent + "40" : COLORS.cardBorder}` }}>
            ...
            <div style={cardStyle}>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                <Link to="/patient/upload" style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", borderRadius: 10, border: `1px solid ${COLORS.cardBorder}`, color: COLORS.text, fontSize: 14, textDecoration: "none", background: COLORS.bg }}>📤 Upload Health Report</Link>
                <button onClick={() => setActiveTab("reports")} style={{ display: "flex", alignItems: "center", gap: 10, padding: "11px 14px", borderRadius: 10, border: `1px solid ${COLORS.cardBorder}`, color: COLORS.text, fontSize: 14, background: COLORS.bg, cursor: "pointer", width: "100%" }}>📋 View All Reports</button>
              </div>
            </div>
          </div>
        </div>
        ...
        <div style={cardStyle}>
          ...
          <p style={{ color: COLORS.muted, fontSize: 13, marginBottom: 14 }}>View your uploaded reports and AI summaries</p>
          <div style={{ display: "flex", gap: 4, marginBottom: 20, background: COLORS.bg, padding: 4, borderRadius: 10, width: "fit-content" }}>
            {[{ key: "reports", label: `All Reports (${displayReports.length})` }, { key: "summary", label: "AI Summary" }].map(tab => (
          ...
          {activeTab === "reports" && (
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
              {displayReports.map((report, idx) => (
                <div key={report.id || idx} onClick={() => { setSelectedReport(report); setActiveTab("summary"); }} style={{ padding: 16, borderRadius: 12, cursor: "pointer", border: `1px solid ${COLORS.cardBorder}`, background: COLORS.bg }} onMouseEnter={e => e.currentTarget.style.borderColor = COLORS.accent} onMouseLeave={e => e.currentTarget.style.borderColor = COLORS.cardBorder}>
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    ...
                    <div style={{ flex: 1 }}>
                      ...
                      <p style={{ color: COLORS.muted, fontSize: 11, marginTop: 6 }}>Uploaded: {report.uploadDate ? new Date(report.uploadDate).toLocaleDateString() : "—"}</p>
                      {/* IPFS View button */}
                      {report.ipfsUrl && (
                        <a href={report.ipfsUrl} target="_blank" rel="noreferrer" onClick={e => e.stopPropagation()} style={{ display: "inline-block", marginTop: 6, fontSize: 11, padding: "3px 10px", borderRadius: 6, background: `${COLORS.green}20`, color: COLORS.green, textDecoration: "none", fontWeight: 600 }}>🌐 View Report</a>
                      )}
                      <button onClick={e => { e.stopPropagation(); handleVerifyReport(report, idx); }} disabled={verifyStatus[idx] === "checking"} style={{ marginTop: 6, marginLeft: report.ipfsUrl ? 6 : 0, fontSize: 11, padding: "3px 10px", borderRadius: 6, cursor: "pointer", border: "none", background: verifyStatus[idx] === "verified" ? `${COLORS.green}22` : verifyStatus[idx] === "failed" ? `${COLORS.red}22` : `${COLORS.accent}22`, color: verifyStatus[idx] === "verified" ? COLORS.green : verifyStatus[idx] === "failed" ? COLORS.red : COLORS.accent }}>
                        {verifyStatus[idx] === "checking" ? "⛓️ Checking..." : verifyStatus[idx] === "verified" ? "✅ Verified" : verifyStatus[idx] === "failed" ? "⚠️ Mismatch" : "🔍 Verify on Chain"}
                      </button>
                    </div>
                  </div>
                </div>
          ...
          {activeTab === "summary" && (
            selectedReport?.aiSummary
              ...
              : <div style={{ textAlign: "center", padding: 48, color: COLORS.muted }}><div style={{ fontSize: 40, marginBottom: 14 }}>📄</div><p>No AI summary available for this report yet.</p></div>
          )}
        </div>
      </div>
    </div>
...
const mockDoctors = [
  { id: "DOC-001", name: "Dr. Ananya Singh", specialty: "Cardiology", hospital: "AIIMS Delhi", rating: 4.8, reviewCount: 247, experience: 12, education: "MBBS, MD (Cardiology)", fee: 800, bio: "Senior Cardiologist specializing in interventional cardiology.", tags: ["Heart Failure","Interventional","ECG"], languages: ["English","Hindi","Punjabi"], availability: ["9:00 AM","10:30 AM","2:00 PM","4:00 PM"], reviews: [] },
  { id: "DOC-002", name: "Dr. Vikram Patel", specialty: "Dermatology", hospital: "Fortis Hospital", rating: 4.5, reviewCount: 183, experience: 8, education: "MBBS, DVD", fee: 600, bio: "Expert in acne, eczema, psoriasis.", tags: ["Acne","Eczema","Cosmetic"], languages: ["English","Gujarati"], availability: ["11:00 AM","1:00 PM","3:30 PM"], reviews: [] },
  { id: "DOC-003", name: "Dr. Meena Roy", specialty: "Orthopedics", hospital: "Apollo Hospital", rating: 4.9, reviewCount: 312, experience: 15, education: "MBBS, MS (Ortho)", fee: 1000, bio: "Joint replacement specialist.", tags: ["Joint Replacement","Spine","Sports Injury"], languages: ["English","Bengali"], availability: ["9:30 AM","11:30 AM","2:30 PM"], reviews: [] },
  { id: "DOC-004", name: "Dr. Suresh Nair", specialty: "Neurology", hospital: "Medanta Hospital", rating: 4.7, reviewCount: 198, experience: 10, education: "MBBS, DM (Neurology)", fee: 900, bio: "Epilepsy and stroke specialist.", tags: ["Epilepsy","Stroke","Migraine"], languages: ["English","Malayalam"], availability: ["10:00 AM","12:00 PM","3:00 PM"], reviews: [] },
  { id: "DOC-005", name: "Dr. Priya Sharma", specialty: "Pediatrics", hospital: "Rainbow Hospital", rating: 4.6, reviewCount: 156, experience: 9, education: "MBBS, MD (Pediatrics)", fee: 500, bio: "Pediatrician with neonatal expertise.", tags: ["Neonatal Care","Vaccinations"], languages: ["English","Hindi"], availability: ["9:00 AM","11:00 AM","4:30 PM"], reviews: [] },
];

const mockReports = [
  { id: "R001", fileName: "Blood_Test_March2026.pdf", category: "Blood Test", uploadDate: "2026-03-01", aiSummary: { keyFindings: ["Cholesterol slightly elevated at 210 mg/dL", "Blood glucose normal at 95 mg/dL"], plainLanguage: "Your blood test shows mostly good results. Cholesterol is slightly high — manageable with diet and exercise.", recommendedSteps: ["Reduce saturated fat intake", "30 min daily exercise", "Follow-up in 3 months"] } },
  { id: "R002", fileName: "Chest_Xray_Feb2026.pdf", category: "X-Ray", uploadDate: "2026-02-14", aiSummary: { keyFindings: ["No abnormalities detected", "Lung fields clear"], plainLanguage: "Your chest X-ray looks completely normal.", recommendedSteps: ["No immediate action required", "Routine check in 1 year"] } },
];
