    ...
    >
      ...
      >
        <div style={{
          ...
          <button onClick={onClose} style={{ position: "absolute", top: 16, right: 16, width: 32, height: 32, borderRadius: 8, background: C.surfaceHi, border: `1px solid ${C.border}`, color: C.muted, fontSize: 16, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>✕</button>
          <div style={{ display: "flex", gap: 20, alignItems: "flex-start" }}>
            <div style={{ width: 80, height: 80, borderRadius: 20, background: `${C.teal}20`, border: `3px solid ${C.teal}50`, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 40 }}>👨‍⚕️</div>
            <div style={{ flex: 1 }}>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          <div style={card}>
            ...
            {(doc.availability||[]).length === 0
              ? <p style={{ color:C.muted, fontSize:13 }}>No slots set.</p>
    ...
    <div>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(2,1fr)", gap:16 }}>
        {filtered.map((doc)=>{
          ...
          return (
            <div key={dk} onClick={() => setSelectedDoc(doc)} style={{ borderRadius:14, border:`1px solid ${C.border}`, background:C.surface, overflow:"hidden", cursor:"pointer", transition:"all 0.15s" }}
            ...
            >
              <div style={{ padding:"18px 20px", display:"flex", gap:14, alignItems:"flex-start" }}>
                <div style={{ width:56, height:56, borderRadius:14, background:`${C.teal}20`, border:`2px solid ${C.teal}40`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:28 }}>👨‍⚕️</div>
                <div style={{ flex:1 }}>
                  ...
                  {doc.rating>0 && <div style={{ display:"flex", alignItems:"center", gap:6 }}><StarRating value={doc.rating}/><span style={{ color:C.amber, fontSize:13, fontWeight:700 }}>{doc.rating}</span></div>}
        ...
        <div style={card}>
          {selectedApt
            ? <div>
                ...
                {[{label:"Patient",value:selectedApt.patientName},{label:"Date",value:selectedApt.date},{label:"Time",value:selectedApt.time},{label:"Type",value:selectedApt.type},{label:"Status",value:selectedApt.status}].map(r=>(
...
function DoctorUploadModal({ patient, doctor, onClose, onSuccess, onShowToast }) {
  ...
  const inputStyle = { width:"100%", padding:"10px 12px", borderRadius:10, fontSize:13, background:C.bg, border:`1px solid ${C.borderHi}`, color:C.text, outline:"none", resize:"vertical", fontFamily:"inherit", boxSizing:"border-box" };

  return (
    <div onClick={onClose} style={{ position:"fixed", inset:0, zIndex:2000, background:"rgba(4,8,15,0.90)", backdropFilter:"blur(8px)", display:"flex", alignItems:"center", justifyContent:"center", padding:24 }}>
      <div onClick={e=>e.stopPropagation()} style={{ width:"100%", maxWidth:620, maxHeight:"90vh", overflowY:"auto", background:C.surface, border:`1px solid ${C.teal}40`, borderRadius:20, boxShadow:`0 32px 80px #000000cc` }}>
        
        {/* Header */}
        <div style={{ background:`linear-gradient(135deg, ${C.teal}15, ${C.blue}10)`, borderBottom:`1px solid ${C.border}`, padding:"22px 28px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <h3 style={{ color:C.text, fontSize:18, fontWeight:800, marginBottom:4 }}>📋 Upload Patient Report</h3>
            <p style={{ color:C.muted, fontSize:13 }}>Uploading report for <span style={{ color:C.teal, fontWeight:700 }}>{patient.name}</span></p>
          </div>
          <button onClick={onClose} style={{ width:32, height:32, borderRadius:8, background:C.surfaceHi, border:`1px solid ${C.border}`, color:C.muted, fontSize:16, cursor:"pointer" }}>✕</button>
        </div>

        {/* Patient info strip */}
        <div style={{ margin:"20px 28px 0", padding:"14px 18px", background:`${C.blue}10`, border:`1px solid ${C.blue}25`, borderRadius:12, display:"flex", gap:16, alignItems:"center" }}>
          <div style={{ width:44, height:44, borderRadius:12, background:`${C.blue}20`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:22 }}>
        ...
        <div style={{ padding:"20px 28px 28px", display:"flex", flexDirection:"column", gap:18 }}>
          ...
          <div>
            ...
            <textarea
              ...
              rows={4}
              placeholder="Write your clinical observations, diagnosis findings, and notes for the patient..."
              style={{ ...inputStyle }}
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={3}
              placeholder="e.g. Rest for 3 days, take prescribed medication, follow up in 1 week..."
              style={{ ...inputStyle }}
            />
          </div>
          ...
          <div>
            ...
            <div
              onClick={() => { const inp = document.createElement("input"); inp.type="file"; inp.accept=".pdf,.jpg,.jpeg,.png"; inp.onchange=e=>setFile(e.target.files[0]); inp.click(); }}
              style={{ padding:"16px 20px", border:`2px dashed ${file?C.teal:C.border}`, borderRadius:12, cursor:"pointer", background:file?`${C.teal}07`:C.bg, textAlign:"center", transition:"all 0.2s" }}
            >
              {file ? (
          ...
          {/* Info note */}
          <div style={{ padding:"12px 16px", background:`${C.teal}08`, border:`1px solid ${C.teal}25`, borderRadius:10, display:"flex", gap:10, alignItems:"flex-start" }}>
            <span style={{ fontSize:16 }}>ℹ️</span>
            <p style={{ color:C.muted, fontSize:12, lineHeight:1.6 }}>
              This report will be immediately visible on the patient's dashboard under "Reports From Doctor". The patient will see your name, the date, your comments, and recommendations.
            </p>
          </div>

          {/* Actions */}
          <div style={{ display:"flex", gap:10, marginTop:4 }}>
            <button onClick={onClose} style={{ flex:1, padding:"11px", borderRadius:10, fontSize:14, fontWeight:600, cursor:"pointer", fontFamily:"inherit", background:"transparent", border:`1px solid ${C.border}`, color:C.muted }}>Cancel</button>
            <button onClick={handleSubmit} disabled={uploading} style={{ flex:2, padding:"11px", borderRadius:10, fontSize:14, fontWeight:700, cursor:uploading?"not-allowed":"pointer", fontFamily:"inherit", background:uploading?C.border:`linear-gradient(135deg,${C.teal},${C.blue})`, border:"none", color:"#fff" }}>
...
function PatientsView({ patients, reports, doctor, onShowToast }) {
  ...
  // Fix: properly filter patients by search query
  const sq = searchQuery.toLowerCase().trim();
  const filtered = (patients || []).filter(p => {
    if (!sq) return true;
    const nameMatch  = (p.name || "").toLowerCase().includes(sq);
    const idMatch    = (p.patientId || p.id || "").toLowerCase().includes(sq);
    const emailMatch = (p.email || "").toLowerCase().includes(sq);
    const phoneMatch = String(p.phone || "").includes(searchQuery.trim());
    return nameMatch || idMatch || emailMatch || phoneMatch;
  });

  const selectedData = (patients || []).find(p => {
    const pid = String(p._id || p.id || p.patientId);
    return pid === selectedPatient;
  });

  // Load patient-specific reports when selection changes
  useEffect(() => {
      ...
      <div style={{ display:"grid", gridTemplateColumns:"1fr 380px", gap:20 }}>
        ...
        <div style={card}>
          <div style={{ position:"relative", marginBottom:16 }}>
            ...
            <input
              ...
              style={{ width:"100%", padding:"11px 14px 11px 38px", borderRadius:10, fontSize:14, background:C.surfaceHi, border:`1px solid ${C.borderHi}`, color:C.text, outline:"none", boxSizing:"border-box" }}
            />
          </div>

          {/* Stats row */}
          <div style={{ display:"flex", gap:8, marginBottom:16 }}>
            <div style={{ flex:1, padding:"10px 14px", background:`${C.teal}0a`, border:`1px solid ${C.teal}20`, borderRadius:10, textAlign:"center" }}>
              <p style={{ color:C.teal, fontSize:18, fontWeight:800, fontFamily:"monospace" }}>{(patients||[]).length}</p>
              <p style={{ color:C.muted, fontSize:11 }}>Total Patients</p>
            </div>
            <div style={{ flex:1, padding:"10px 14px", background:`${C.blue}0a`, border:`1px solid ${C.blue}20`, borderRadius:10, textAlign:"center" }}>
              <p style={{ color:C.blue, fontSize:18, fontWeight:800, fontFamily:"monospace" }}>{filtered.length}</p>
              <p style={{ color:C.muted, fontSize:11 }}>Showing</p>
            </div>
            <div style={{ flex:1, padding:"10px 14px", background:`${C.purple}0a`, border:`1px solid ${C.purple}20`, borderRadius:10, textAlign:"center" }}>
              <p style={{ color:C.purple, fontSize:18, fontWeight:800, fontFamily:"monospace" }}>{reports.length}</p>
              <p style={{ color:C.muted, fontSize:11 }}>Total Reports</p>
            </div>
          </div>
          ...
          {filtered.length === 0
            ? <div style={{ textAlign:"center", padding:"32px 0", color:C.muted }}>
                ...
                {searchQuery && <p style={{ fontSize:12, marginTop:6, color:C.mutedHi }}>Try searching by name, ID (e.g. HLT-0x...), email or phone</p>}
              </div>
                ...
                {filtered.map(p => {
                  ...
                  return (
                    <div
                      ...
                      style={{ padding:"13px 14px", borderRadius:10, cursor:"pointer", marginBottom:8, border:`1px solid ${isSelected?C.teal:C.border}`, background:isSelected?`${C.teal}08`:C.surfaceHi, display:"flex", alignItems:"center", justifyContent:"space-between", transition:"all 0.15s" }}
                    >
                      <div style={{ display:"flex", alignItems:"center", gap:12 }}>
                        <div style={{ width:40, height:40, borderRadius:10, background:isSelected?`${C.teal}20`:`${C.blue}15`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20 }}>
                          {p.gender === "Female" ? "👩" : "👨"}
                        </div>
                        <div>
        ...
        <div style={{ display:"flex", flexDirection:"column", gap:16 }}>
          {selectedData ? (
            <>
              ...
              <div style={{ ...card, padding:20 }}>
                <div style={{ textAlign:"center", paddingBottom:16, borderBottom:`1px solid ${C.border}`, marginBottom:16 }}>
                  <div style={{ width:64, height:64, borderRadius:16, background:`${C.teal}20`, border:`2px solid ${C.teal}40`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:30, margin:"0 auto 10px" }}>
                ...
                <button
                  ...
                  style={{ width:"100%", padding:"12px", borderRadius:10, border:"none", cursor:"pointer", fontSize:14, fontWeight:700, fontFamily:"inherit", background:`linear-gradient(135deg,${C.teal},${C.blue})`, color:"#fff", display:"flex", alignItems:"center", justifyContent:"center", gap:8 }}
                >
                    ...
                    {patientReportsLocal.map((r, i) => (
                      <div key={r.id || i} style={{ padding:"12px 14px", background:C.bg, borderRadius:10, border:`1px solid ${C.border}` }}>
                        <div style={{ display:"flex", gap:10, alignItems:"flex-start" }}>
                          <div style={{ width:34, height:34, borderRadius:8, background:`${C.blue}20`, display:"flex", alignItems:"center", justifyContent:"center", fontSize:16, flexShrink:0 }}>📄</div>
                          <div style={{ flex:1 }}>
            ...
            <div style={{ ...card, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:300, textAlign:"center", color:C.muted }}>
              <div style={{ fontSize:40, marginBottom:12 }}>👆</div>
              <p style={{ fontSize:15, fontWeight:600, color:C.text, marginBottom:8 }}>Select a patient</p>
              <p style={{ fontSize:13, lineHeight:1.6 }}>Search and select a patient from the list to view their records or upload a new report with your clinical notes.</p>
            </div>
          )}
        </div>
      </div>
...
function MyProfileView({ doctor: doc, onUpdateProfile, licenseVerification, onLicenseVerify, onShowToast }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = { bio, hospital, education, experience: Number(experience), fee: Number(fee), specialty, phone, availability: flatAvailability, availabilityMap: availability, languages: languagesInput.split(",").map(s=>s.trim()).filter(Boolean), tags: tagsInput.split(",").map(s=>s.trim()).filter(Boolean) };
          ...
          {editMode ? (
            <div style={card}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12, marginBottom:12 }}>
                {[{label:"Specialty",val:specialty,set:setSpecialty},{label:"Hospital",val:hospital,set:setHospital},{label:"Experience (years)",val:experience,set:setExperience,type:"number"},{label:"Fee (₹)",val:fee,set:setFee,type:"number"},{label:"Phone",val:phone,set:setPhone},{label:"Education",val:education,set:setEducation}].map(f=>(
                  <div key={f.label}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:12 }}>
                {[{label:"Languages (comma-separated)",val:languagesInput,set:setLanguagesInput},{label:"Conditions/Tags",val:tagsInput,set:setTagsInput}].map(f=>(
                  <div key={f.label}>
                    <label style={{ color:C.muted, fontSize:11, display:"block", marginBottom:4 }}>{f.label}</label>
                    <input value={f.val} onChange={e=>f.set(e.target.value)} style={{ ...inputStyle, resize:"none" }}/>
                  </div>
                ))}
              </div>
            </div>
            ...
            <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:14 }}>
                {[{label:"🎓 Education",value:doc.education||"Not set"},{label:"🏥 Hospital",value:doc.hospital||"Not set"},{label:"⏱️ Experience",value:doc.experience?`${doc.experience} years`:"Not set"},{label:"💰 Fee",value:doc.fee?`₹${doc.fee}`:"Not set"},{label:"📱 Phone",value:doc.phone||"Not set"},{label:"📜 License",value:doc.licenseNumber||"Not set"}].map(r=>(
...
export function DoctorDashboard() {
  ...
  useEffect(()=>{
    ...
    if(id && id!=="USR-LOCAL") {
      fetch(`${API}/doctors/${id}`).then(r=>r.ok?r.json():null).then(d=>{ if(d) setDoctorProfile(d); }).catch(()=>{});
    }
  }, []);
  ...
  const handleCompleteAppt=async(appt)=>{
    ...
    try {
      ...
      show(`Appointment marked complete.`,"info");
      setAppointments(prev=>prev.map(a=>a.id===appt.id?{...a,status:"completed"}:a));
    }
  };
