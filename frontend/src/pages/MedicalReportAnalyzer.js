...
const LANG_CODES = {
  "English":"en-US","British English":"en-GB","Hindi":"hi-IN",
  "Bengali":"bn-IN","French":"fr-FR","Chinese (Simplified)":"zh-CN",
  "Spanish":"es-ES","German":"de-DE","Arabic":"ar-SA","Tamil":"ta-IN",
  "Telugu":"te-IN","Marathi":"mr-IN","Gujarati":"gu-IN","Punjabi":"pa-IN",
  "Urdu":"ur-PK","Japanese":"ja-JP","Korean":"ko-KR","Italian":"it-IT",
...
const statusColor = (s="") => { s=s.toLowerCase(); return s==="high"?C.red:s==="low"?C.yellow:s==="normal"?C.green:C.muted; };
const statusIcon  = (s="") => { s=s.toLowerCase(); return s==="high"?"↑":s==="low"?"↓":s==="normal"?"✓":"•"; };

// ── Call backend for report analysis (Groq) ───────────────────────────────────
async function analyzeReport(payload) {
  const MAX = 8000;
  let body;
  if (payload.imageBase64) {
    body = { imageBase64: payload.imageBase64, imageMimeType: payload.imageMimeType,
      reportType: payload.reportType, preferredLanguage: payload.preferredLanguage,
      explainLevel: payload.explainLevel, voiceFriendly: payload.voiceFriendly };
  } else {
    body = { reportText: payload.reportText.length > MAX ? payload.reportText.slice(0,MAX)+"\n[Truncated]" : payload.reportText,
      reportType: payload.reportType, preferredLanguage: payload.preferredLanguage,
      explainLevel: payload.explainLevel, voiceFriendly: payload.voiceFriendly };
  }
...
// ── Call backend /api/tts (Gemini TTS) → returns audio blob ──────────────────
async function fetchTTSAudio(text, langCode) {
  const res = await fetch(`${BACKEND_URL}/api/tts`, {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ text: text.slice(0, 1000), langCode }),
  });
  if (!res.ok) {
    const e = await res.json().catch(() => ({}));
    throw new Error(e.error || `TTS error: ${res.status}`);
  }
  return res.blob(); // audio blob (wav/mp3)
}
...
async function readFile(file) {
  ...
  if (imageTypes.includes(file.type)) {
    return new Promise((resolve,reject) => {
      ...
      r.onload = e => resolve({ type:"image", base64:e.target.result, mimeType:file.type, fileName:file.name });
  ...
  if (file.type==="text/plain" || file.name.endsWith(".txt")) {
    return new Promise((resolve,reject) => {
      const r = new FileReader();
      r.onload = e => resolve({ type:"text", text:e.target.result, fileName:file.name });
  ...
  return new Promise((resolve,reject) => {
    ...
    r.onload = e => {
      ...
      try {
        ...
        const parts=[];
        matches.forEach(b=>{
          (b.match(/\(([^)]+)\)\s*Tj/g)||[]).forEach(tj=>{ const m=tj.match(/\(([^)]+)\)/); if(m&&m[1].trim()) parts.push(m[1].trim()); });
        });
        text = parts.join(" ").trim();
        if (!text) { text=raw.replace(/[^\x20-\x7E\n\r\t]/g," ").replace(/\s+/g," ").trim(); text=text.split(/\s{3,}/).filter(l=>l.length>5&&/[a-zA-Z]/.test(l)).slice(0,80).join("\n"); }
      } catch { text=""; }
      if (text.length<30) resolve({type:"text",text:`[PDF: ${file.name}]\nCould not extract text. Please paste manually.`,fileName:file.name});
      else resolve({type:"text",text:`[PDF: ${file.name}]\n\n${text.length>8000?text.slice(0,8000)+"\n[Truncated]":text}`,fileName:file.name});
    };
...
// ── UI helpers ────────────────────────────────────────────────────────────────
function SectionLabel({children}){return <p style={{color:C.muted,fontSize:10,textTransform:"uppercase",letterSpacing:1.5,fontWeight:700,marginBottom:10,fontFamily:"monospace"}}>{children}</p>;}
function Pill({color,children}){return <span style={{background:`${color}18`,color,border:`1px solid ${color}35`,padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:700}}>{children}</span>;}
    ...
    <div style={{overflowX:"auto"}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
        <thead><tr style={{borderBottom:`1px solid ${C.cardBorder}`}}>
          {["Parameter","Value","Status","Meaning"].map(h=><th key={h} style={{padding:"8px 12px",textAlign:"left",color:C.muted,fontWeight:600,fontSize:11,textTransform:"uppercase",letterSpacing:1}}>{h}</th>)}
        </tr></thead>
        <tbody>
          {values.map((v,i)=>(
            <tr key={i} style={{borderBottom:`1px solid ${C.cardBorder}20`}}>
              ...
              <td style={{padding:"10px 12px"}}><span style={{color:statusColor(v.status),fontWeight:700,fontSize:12,background:`${statusColor(v.status)}15`,padding:"2px 8px",borderRadius:8}}>{statusIcon(v.status)} {v.status}</span></td>
...
// ── VoicePlayer — uses Gemini TTS via backend, 100% browser-independent ───────
function VoicePlayer({ text, langCode, language }) {
  ...
  const handlePlay = async () => {
    ...
    // If loading — ignore double click
    ...
    try {
      // 1. Call backend /api/tts → Gemini converts text to audio
      const blob = await fetchTTSAudio(text, langCode);

      // 2. Create a blob URL and play it
      if (blobUrlRef.current) URL.revokeObjectURL(blobUrlRef.current);
      ...
      audio.onerror  = () => { setStatus("error"); setErrMsg("Audio playback failed."); };
  ...
  return (
    <div style={{ background:`${C.accent2}10`, border:`1px solid ${C.accent2}30`, borderRadius:12, padding:16, marginTop:12 }}>
      <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}>
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          ...
          <div>
            ...
            <span style={{ marginLeft:8, background:`${C.green}15`, color:C.green, border:`1px solid ${C.green}30`, padding:"2px 8px", borderRadius:10, fontSize:10, fontWeight:700 }}>
              Gemini TTS · {language}
            </span>
          </div>
        </div>
        <button
          ...
          style={{
            ...
            background: isPlaying ? `${C.red}22` : isLoading ? `${C.yellow}22` : `${C.accent2}22`,
            color: isPlaying ? C.red : isLoading ? C.yellow : C.accent2,
      ...
      {isPlaying && (
        <div style={{ display:"flex", alignItems:"center", gap:3, marginBottom:10, height:20 }}>
          {[1,2,3,4,5,6,7,8].map(i => (
            <div key={i} style={{
              width:3, borderRadius:2,
              background:`linear-gradient(${C.accent2}, ${C.accent})`,
              animation:`wave 0.8s ease-in-out infinite`,
              animationDelay:`${i*0.1}s`,
              height: `${8 + (i%3)*6}px`,
            }}/>
          ))}
          <span style={{ color:C.muted, fontSize:11, marginLeft:8 }}>Speaking in {language}…</span>
        </div>
      )}

      {/* Error */}
      {isError && (
        <div style={{ padding:"8px 12px", borderRadius:8, background:`${C.red}12`, border:`1px solid ${C.red}30`, marginBottom:10 }}>
          <p style={{ color:C.red, fontSize:11 }}>❌ {errMsg}</p>
          <p style={{ color:C.muted, fontSize:10, marginTop:3 }}>Make sure GEMINI_API_KEY is set in backend/.env</p>
        </div>
      )}
      ...
      <div style={{ padding:"16px 20px", background:`linear-gradient(135deg, ${C.accent}12, ${C.accent2}12)`, borderBottom:`1px solid ${C.cardBorder}`, display:"flex", justifyContent:"space-between", alignItems:"center" }}>
        <div style={{ display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div>
            ...
            <p style={{ color:C.muted, fontSize:11 }}>Summary: Groq LLaMA · Voice: Gemini TTS · Language: {result.language}</p>
          </div>
        </div>
      ...
      <div style={{ padding:20 }}>
        {tab==="summary" && (
          <div>
            ...
            {/* Gemini TTS VoicePlayer — no browser voice needed */}
            {result.voiceText && <VoicePlayer text={result.voiceText} langCode={langCode} language={result.language} />}
...
function UploadBox({ onFileRead, onError }) {
  ...
  const handleFile = async (file) => {
    ...
    if (!allowed.includes(file.type)&&!file.name.endsWith(".txt")&&!file.name.endsWith(".pdf")) { onError("Please upload PDF, PNG, JPG, or TXT."); return; }
    ...
    try { const r=await readFile(file); onFileRead(r); }
  ...
  return (
    <div onDragOver={e=>{e.preventDefault();setDragOver(true);}} onDragLeave={()=>setDragOver(false)}
      ...
      {extracting ? <div><div style={{fontSize:28,marginBottom:8}}>⏳</div><p style={{color:C.accent,fontSize:13,fontWeight:600}}>Reading file…</p></div>
      : fileName ? <div>
      ...
      : <div>
          ...
          <p style={{color:C.muted,fontSize:12}}>PDF, PNG, JPG, TXT · Max 10MB</p>
          <div style={{display:"flex",gap:8,justifyContent:"center",marginTop:10,flexWrap:"wrap"}}>
            {["📄 PDF","🖼️ PNG","🖼️ JPG","📝 TXT"].map(t=><span key={t} style={{background:`${C.accent}12`,color:C.accent,border:`1px solid ${C.accent}30`,padding:"3px 10px",borderRadius:20,fontSize:11,fontWeight:600}}>{t}</span>)}
          </div>
        </div>}
    </div>
  );
...
export default function MedicalReportAnalyzer() {
  ...
  const handleFileRead = (fileResult) => {
    ...
    } else { setImageData(null); setReportText(fileResult.text); }
  };
  ...
  const handleAnalyze = async () => {
    ...
    try {
      const parsed = await analyzeReport({
        ...
        reportType, preferredLanguage:language, explainLevel, voiceFriendly,
      });
      clearInterval(progressRef.current); setProgress(100);
      setTimeout(()=>{ setLoading(false); setResult(parsed); }, 300);
    } catch(err) {
      clearInterval(progressRef.current); setLoading(false); setProgress(0);
      ...
      else setError("Analysis failed: "+err.message);
    }
  };

  const handleReset = () => { setReportText(""); setImageData(null); setResult(null); setError(""); setProgress(0); };
  ...
  return (
    <div style={{ minHeight:"100vh", background:C.bg, fontFamily:"'Segoe UI', -apple-system, sans-serif", color:C.text }}>
      ...
      <div style={{ background:"#080d1a", borderBottom:`1px solid ${C.cardBorder}`, padding:"0 32px", height:64, display:"flex", alignItems:"center", justifyContent:"space-between", position:"sticky", top:0, zIndex:100 }}>
        ...
        <div style={{ display:"flex", alignItems:"center", gap:8 }}>
          <span style={{ background:`${C.green}15`, color:C.green, border:`1px solid ${C.green}30`, padding:"4px 12px", borderRadius:20, fontSize:11, fontWeight:700 }}>🟢 Groq + Gemini</span>
      ...
      <div style={{ maxWidth:860, margin:"0 auto", padding:"32px 24px" }}>
        <div style={{ marginBottom:28 }}>
          ...
          <p style={{ color:C.muted, fontSize:14, lineHeight:1.6 }}>
            Upload PDF, PNG, JPG or paste text. <strong style={{color:C.accent}}>Groq AI</strong> generates the summary · <strong style={{color:C.accent2}}>Gemini TTS</strong> speaks it in any language — no browser voices needed.
          </p>
        </div>
        ...
        {!result && (
          <>
            ...
            <div style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:16, padding:20, marginBottom:20 }}>
              ...
              {(inputMode==="text"||(inputMode==="file"&&!imageData)) && (
                <>
                  ...
                  <p style={{ color:C.muted, fontSize:13, marginBottom:12 }}>{inputMode==="file"?"Extracted text — edit before analyzing":"Paste from lab report, prescription, or medical document"}</p>
                  <textarea value={reportText} onChange={e=>setReportText(e.target.value)}
                    placeholder={"Example:\nHemoglobin: 10 g/dL (Low)\nWBC: 8,500 /μL (Normal)\nBlood Glucose: 180 mg/dL (High)\n\nPaste any medical report text here..."}
                    rows={10} style={{...inputStyle,resize:"vertical",lineHeight:1.7,fontFamily:"monospace",fontSize:13}}/>
                  <div style={{ display:"flex", justifyContent:"space-between", marginTop:8 }}>
                    <span style={{ color:reportText.length>8000?C.red:C.muted, fontSize:11 }}>{reportText.length} chars {reportText.length>8000&&"⚠️ will be trimmed"}</span>
                    {reportText.length>0&&<button onClick={()=>{setReportText("");setImageData(null);}} style={{ background:"none", border:"none", color:C.red, cursor:"pointer", fontSize:12 }}>✕ Clear</button>}
                  </div>
                </>
              )}
            ...
            <div style={{ marginBottom:20, textAlign:"center" }}>
              <button onClick={()=>{ setInputMode("text"); setImageData(null);
                setReportText(`Patient: John Doe | Age: 45 | Date: ${new Date().toLocaleDateString()}\nReport Type: CBC\n\nHemoglobin: 10.2 g/dL [Normal: 13.5-17.5] - LOW\nRBC: 3.8 million/μL [Normal: 4.5-5.5] - LOW\nWBC: 11,500 /μL [Normal: 4,500-11,000] - HIGH\nPlatelet: 145,000 /μL [Normal: 150,000-400,000] - LOW\nHematocrit: 31% [Normal: 41-53%] - LOW\n\nDoctor's Note: Fatigue and shortness of breath.`);
            ...
            {error&&<div style={{ padding:14, borderRadius:10, marginBottom:16, background:`${C.red}10`, border:`1px solid ${C.red}30`, color:C.red, fontSize:13, lineHeight:1.8, whiteSpace:"pre-line" }}>⚠️ {error}</div>}
            ...
            {!loading ? (
              ...
              <div style={{ background:C.card, border:`1px solid ${C.cardBorder}`, borderRadius:12, padding:20 }}>
                <div style={{ display:"flex", justifyContent:"space-between", marginBottom:10 }}>
                  <span style={{ color:C.text, fontSize:14, fontWeight:600 }}>{progress<30?"Reading file…":progress<60?"Analyzing with Groq AI…":progress<85?"Generating summary…":"Finalizing…"}</span>
                ...
                <p style={{ color:C.muted, fontSize:12, marginTop:8, textAlign:"center" }}>Groq AI analyzing in {language}…</p>
              </div>
            )}
          </>
        )}
        ...
        {result && (
          <>
            ...
            <div style={{ display:"flex", gap:14, marginTop:20 }}>
              ...
              <button onClick={()=>{ const b=new Blob([JSON.stringify(result,null,2)],{type:"application/json"}); const a=document.createElement("a"); a.href=URL.createObjectURL(b); a.download="medichain-analysis.json"; a.click(); }} style={{ flex:1, padding:14, borderRadius:11, border:"none", background:`linear-gradient(135deg, ${C.green}, #059669)`, color:"#fff", fontSize:14, fontWeight:700, cursor:"pointer" }}>⬇ Download Analysis (JSON)</button>
            </div>
          </>
        )}
      </div>
